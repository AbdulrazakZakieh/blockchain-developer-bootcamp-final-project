...
async function getDisputeableContractInfo(){
  ...
  try{
    ...
    const response = await disputeableContract.methods.showDisputeableContract(contract_id).send({from: ethereum.selectedAddress});
    console.log(response)
    document.getElementById('contract_information').innerHTML += '<p>' + response + '</p>';
  } catch(err){
    console.log(err)
    alert(err.message)
  } 
